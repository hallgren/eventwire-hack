...
class Eventwire::Drivers::AMQP_UPPTEC
  ...
  def publish(event_name, event_data = nil)
    EMWrapper.new do |em|
      connection = AMQP.connect(:host => APP_CONFIG["eventwire_host"])
      channel  = AMQP::Channel.new(connection)
      exchange = channel.topic(APP_CONFIG["eventwire_exchange"], :durable => true)

      exchange.publish(event_data, :routing_key => event_name, :persistent => true, :durable => true) do
